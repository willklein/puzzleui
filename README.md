...
| Keys                     | Behavior                                                                                                                                                                                                                                                                                                                                  |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Arrow keys               | Move focus one cell, clamped at the grid edges (not wrapped). Collapses an active multi-selection down to just the newly-focused cell.                                                                                                                                                                                                    |
| Digit key (`1`..size)    | Sets the cell's value when `noteMode` is off; toggles a plain candidate note when `noteMode` is on. With a multi-selection active (see Cmd/Ctrl + Click below), applies to every selected cell at once, as a single undo step.                                                                                                            |
| `Shift` + digit key      | Toggles a plain candidate note, even while `noteMode` is off — a shortcut for jotting one note without leaving value-entry mode. While `noteMode` IS on, Shift instead marks/re-tags the note as highlighted using the current `highlightMode` (`'box'`/`'row'`/`'col'`). Also fans out across a multi-selection, like a plain digit key. |
| `Backspace` / `Delete`   | On a valued cell, clears its value and restores the notes it had right before being filled in, independent of anything else in the undo/redo history since. On an empty cell, clears its notes. With a multi-selection active, applies per-cell across the whole selection. (Undo/Redo buttons still walk the full history.)              |
| Click / tap              | Focuses the cell, replacing any active multi-selection; if its notes (established via `autoNote()`) have narrowed to exactly one remaining candidate (`autoSolveOnClick`, on by default), commits it immediately — a forced cell the assist could solve from constraint math alone, but that was never noted, does not auto-solve.        |
| Cmd (Mac) / Ctrl + Click | Toggles the clicked cell into or out of a multi-selection alongside whichever cell(s) were already selected, and focuses it — never triggers auto-solve. Digit keys, Shift+digit, and Backspace then act on every selected cell.                                                                                                          |
...
| ----------------------------------------------------- | ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `shouldSetValue` / `onSetValue`                       | `{ index, digit: number \| null }` | Committing/clearing a cell — `setValue()`, digit key, Backspace, click auto-solve. On a multi-selection, also covers Backspace clearing residual notes on an already-empty selected cell. |
